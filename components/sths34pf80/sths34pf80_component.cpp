...
int32_t Sths34pf80Component::apply_cached_parameters() {
  if (this->state_.is_uninitialized()) {
    ESP_LOGW(TAG, "Can not apply cached parameters while the component is uninitialized.");
  }
  ...
  if (this->parameter_cache_.avg_t_object_number.has_value()) {
    result = sths34pf80_avg_tobject_num_set(&this->sensor_, this->parameter_cache_.avg_t_object_number.value());
    if (result != 0) {
  ...
  if (this->parameter_cache_.avg_t_ambient_number.has_value()) {
    result = sths34pf80_avg_tambient_num_set(&this->sensor_, this->parameter_cache_.avg_t_ambient_number.value());
    if (result != 0) {
  ...
  if (this->parameter_cache_.gain_mode.has_value()) {
    result = sths34pf80_gain_mode_set(&this->sensor_, this->parameter_cache_.gain_mode.value());
    if (result != 0) {
  ...
  // Apply cached LPF bandwidths
  if (this->parameter_cache_.lpf_motion_bandwidth.has_value()) {
    result = sths34pf80_lpf_m_bandwidth_set(&this->sensor_, this->parameter_cache_.lpf_motion_bandwidth.value());
    if (result != 0) {
      ESP_LOGW(TAG, "Failed to apply cached LPF motion bandwidth");
    }
  }

  if (this->parameter_cache_.lpf_presence_motion_bandwidth.has_value()) {
    result =
        sths34pf80_lpf_p_m_bandwidth_set(&this->sensor_, this->parameter_cache_.lpf_presence_motion_bandwidth.value());
    if (result != 0) {
      ESP_LOGW(TAG, "Failed to apply cached LPF presence motion bandwidth");
    }
  }

  if (this->parameter_cache_.lpf_ambient_temp_bandwidth.has_value()) {
    result =
        sths34pf80_lpf_a_t_bandwidth_set(&this->sensor_, this->parameter_cache_.lpf_ambient_temp_bandwidth.value());
    if (result != 0) {
      ESP_LOGW(TAG, "Failed to apply cached LPF ambient temp bandwidth");
    }
  }

  if (this->parameter_cache_.lpf_presence_bandwidth.has_value()) {
    result = sths34pf80_lpf_p_bandwidth_set(&this->sensor_, this->parameter_cache_.lpf_presence_bandwidth.value());
    if (result != 0) {
  ...
  if (this->parameter_cache_.presence_threshold_lsb.has_value()) {
    result = sths34pf80_presence_threshold_set(&this->sensor_, this->parameter_cache_.presence_threshold_lsb.value());
    if (result != 0) {
      ESP_LOGW(TAG, "Failed to apply cached presence threshold");
    }
  }

  if (this->parameter_cache_.motion_threshold_lsb.has_value()) {
    result = sths34pf80_motion_threshold_set(&this->sensor_, this->parameter_cache_.motion_threshold_lsb.value());
    if (result != 0) {
      ESP_LOGW(TAG, "Failed to apply cached motion threshold");
    }
  }

  if (this->parameter_cache_.t_ambient_shock_threshold_lsb.has_value()) {
    result = sths34pf80_tambient_shock_threshold_set(&this->sensor_,
                                                     this->parameter_cache_.t_ambient_shock_threshold_lsb.value());
    if (result != 0) {
  ...
  if (this->parameter_cache_.motion_hysteresis_lsb.has_value()) {
    result = sths34pf80_motion_hysteresis_set(&this->sensor_, this->parameter_cache_.motion_hysteresis_lsb.value());
    if (result != 0) {
      ESP_LOGW(TAG, "Failed to apply cached motion hysteresis");
    }
  }

  if (this->parameter_cache_.presence_hysteresis_lsb.has_value()) {
    result = sths34pf80_presence_hysteresis_set(&this->sensor_, this->parameter_cache_.presence_hysteresis_lsb.value());
    if (result != 0) {
      ESP_LOGW(TAG, "Failed to apply cached presence hysteresis");
    }
  }

  if (this->parameter_cache_.t_ambient_shock_hysteresis_lsb.has_value()) {
    result = sths34pf80_tambient_shock_hysteresis_set(&this->sensor_,
                                                      this->parameter_cache_.t_ambient_shock_hysteresis_lsb.value());
    if (result != 0) {
  ...
  if (this->parameter_cache_.t_object_algo_compensation.has_value()) {
    result = sths34pf80_tobject_algo_compensation_set(&this->sensor_,
                                                      this->parameter_cache_.t_object_algo_compensation.value());
    if (result != 0) {
  ...
  if (this->parameter_cache_.presence_abs_value.has_value()) {
    uint8_t abs_value = this->parameter_cache_.presence_abs_value.value() == PRESENCE_ABS_ENABLED ? 1 : 0;
  ...
  // this needs to be done last as it take the device out of power down mode, and
  // set it into continuous mode

  /*sths34pf80_tmos_odr_t val = this->parameter_cache_.tmos_odr.value_or(STHS34PF80_TMOS_ODR_AT_1Hz);
  this->odr_ms_ = (uint32_t) get_odr_interval(val);

  result = sths34pf80_tmos_odr_set(&this->sensor_, val);
  if (result != 0) {
    ESP_LOGW(TAG, "Failed to apply cached ODR");
  }*/
  this->start_sampling();
  return 0;
}
...
int32_t Sths34pf80Component::restart_device() {
  // Note: reset and configring algothem is required after calling this
  ...
  return sths34pf80_tmos_odr_set(&this->sensor_, STHS34PF80_TMOS_ODR_OFF);
}
...
int32_t Sths34pf80Component::stop_sampling() {
  ...
  int32_t result = sths34pf80_tmos_odr_set(&this->sensor_, STHS34PF80_TMOS_ODR_OFF);
  if (result != 0) {
    return result;
  }
...
int32_t Sths34pf80Component::start_sampling() {
  ...
  sths34pf80_tmos_odr_t odr = this->parameter_cache_.tmos_odr.value_or(STHS34PF80_TMOS_ODR_AT_1Hz);
  if (odr == STHS34PF80_TMOS_ODR_OFF) {
    odr = STHS34PF80_TMOS_ODR_AT_1Hz;
  }
